...
export const CafesHero = () => {
  ...
  useEffect(() => {
    ...
    console.log("Current image URL:", backgroundImages[currentImageIndex].url);
  }, [currentImageIndex]);
    ...
    <section className="relative min-h-[80vh] flex items-center overflow-hidden" style={{ backgroundColor: '#983a45' }}>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          onError={() => console.log("Error loading background image:", backgroundImages[currentImageIndex].url)}
        />
      </AnimatePresence>
      ...
      <div className="absolute inset-0 opacity-15 pointer-events-none z-10">
        ...
        <div className="absolute bottom-20 right-32 w-8 h-8">
          <Icon icon="lucide:users" className="text-white text-2xl -rotate-30" />
        </div>
        ...
        <div className="absolute top-3/4 left-1/2 w-6 h-6">
          <Icon icon="lucide:map-pin" className="text-white text-xl rotate-180" />
        </div>
